...
function choveCoracao(){for(var a=[];4>a.length;){for(var b=Math.floor(12*Math.random()),d=!1,c=0;c<a.length;c++)if(a[c]==b){d=!0;break}d||(a[a.length]=b)}for(x=0;x<a.length;x++)b=cors.create(70*a[x],0,"coracao"),b.body.gravity.y=6+3*Math.floor(Math.random()),b.body.bounce.y=.9+.2*Math.random()}function balancaCoracao(a){a.body.velocity.x=2>a.x?Math.floor(80*Math.random())-50:10}
...
function enviaRecorde(){$.ajax({type:"GET",crossDomain:!0,dataType:"jsonp",url:"http://jogovacilo.2fh.co/recorde.php",data:{nome:nomeVacilao,tempo:tempoPerdido},dataType:"jsonp",timeout:1E4,success:function(a){bateuRecorde(a)}})}function imprimeTempoPerdido(){seFudeu||dancaDaVitoria||(tempoPerdido=(new Date).getTime()-inicioJogo);var a,b;x=tempoPerdido;x/=1E3;a=Math.floor(x%60);x/=60;b=Math.floor(x%60);x/=60;return[pad2(Math.floor(x%24)),pad2(b),pad2(a)].join(":")}
function pad2(a){return(10>a?"0":"")+a};
